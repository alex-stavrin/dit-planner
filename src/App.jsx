...
import { coursesDataVersion } from './data/coursesDataVersion';
import { useToast } from './hooks/use-toast';
import { AllCourses } from './pages/AllCourses';
// To export course data
...
function App() {
  ...
  const makeCourse = (name, ECTS) => {
    let newCourse = {
      name,
      code: 'MC' + name.substring(0, 2).toUpperCase() + courses.length,
      ECTS,
      category: 'MC',
      hasCourse: true,
      userMade: true,
  ...
  const importFromFile = (file) => {
    ...
    reader.onload = (event) => {
      ...
      showToast('Data Imported', 'Your courses data has been successfully imported.', 'success');
    };
    reader.onerror = () => {
      showToast('Import Failed', 'There was an error importing the data.', 'error');
    };
  ...
  const removeHasCourse = (removedCourse) => {
    if (removedCourse.category === 'MC') {
      ...
      showToast(
        'Custom Course Removed',
        'Removed ' + removedCourse.name + ' from courses because you dropped it.',
        'info'
      );
    } else {
      setCourses((prevCourses) =>
        prevCourses.map((course) =>
          course.code === removedCourse.code
            ? { ...course, hasCourse: false, grade: 0, isActive: false }
            : course
        )
      );
      showToast(
        'Course Removed',
        'Removed ' + removedCourse.name + ' from planned courses',
        'info'
      );
    }
  };
  ...
  const changeGrade = (newGrade, changedCourse) => {
    if (newGrade >= 0 && newGrade <= 10) {
      if (newGrade < 5 && changedCourse.grade >= 5) {
        showToast(
          'Removed Passed Course',
          'Removed ' + changedCourse.name + ' from passed courses',
          'info'
        );
      }
      setCourses((prevCourses) =>
      ...
      if (newGrade >= 5) {
        showToast(
          'Passed Course!',
          'Added ' + changedCourse.name + ' to passed courses',
          'success'
        );
      }
    }
  ...
  const updateActivity = (newActivity, updatedCourse) => {
    ...
    if (newActivity) {
      showToast(
        'Added current course',
        'Added ' + updatedCourse.name + ' to current courses',
        'success'
      );
    } else {
      showToast(
        'Removed current course',
        'Removed ' + updatedCourse.name + ' from current courses',
        'info'
      );
    }
  };
  ...
  const syncSavedWithCourseData = () => {
    ...
    if (notParsedCourses) {
      ...
      showToast(
        'Synced Data',
        'Changes were made in the course data. Your data is synced. Look for any errors though',
        'success'
      );
    } else {
  ...
  const noPassedComponent = () => {
    return (
      <div className="mt-5 flex h-full w-full items-center justify-center">
        <p>
          You have no passed classes. With a grade higher or equal to 5 it will be added here. Grade
          your classes in{' '}
          <ReactRouterLink to="/dit-planner/planned" className="text-blue-500">
            planned courses.
          </ReactRouterLink>{' '}
  ...
  return (
    <div className={`flex h-full w-full flex-col ${theme === 'dark' ? 'dark' : ''}`}>
      <BurgerHeader />
      <div className="overflow-auto">
        <Routes>
          ...
          <Route
            path="/dit-planner/current"
            element={
              <CurrentCourses
                courses={courses}
                removeHasCourse={removeHasCourse}
                changeGrade={changeGrade}
                updateActivity={updateActivity}
                currentCourseState={currentCourseState}
                showActivity={true}
                showGrade={false}
                noCurrentComponent={noCurrentComponent}
              />
            }
          />

          <Route
            path="/dit-planner/passed"
            element={
              <PassedCourses
                courses={courses}
                removeHasCourse={removeHasCourse}
                changeGrade={changeGrade}
                updateActivity={updateActivity}
                passedCourseState={passedCourseState}
                noPassedComponent={noPassedComponent}
              />
            }
          />

          <Route
            path="/dit-planner/planned"
            element={
              <PlannedCourses
                courses={courses}
                removeHasCourse={removeHasCourse}
                changeGrade={changeGrade}
                updateActivity={updateActivity}
                plannedCourseState={plannedCourseState}
                showActivity={true}
                showGrade={false}
                noPlannedComponent={noPlannedComponent}
              />
            }
          />

          <Route
            path="/dit-planner/all"
            element={<AllCourses courses={courses} onAdd={addHasCourse} />}
          ></Route>

          <Route path="/dit-planner/make" element={<MakeCourse onMakeCourse={makeCourse} />} />

          <Route
            path="/dit-planner/settings"
            element={
              <Settings
                onResetData={resetData}
                onSyncData={syncSavedWithCourseData}
                onExportData={() => exportToFile(courses)}
                onImportData={importFromFile}
                version={coursesDataVersion}
              />
            }
          />
        </Routes>
